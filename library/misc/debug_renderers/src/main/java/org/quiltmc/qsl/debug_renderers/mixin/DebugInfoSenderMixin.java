...
public abstract class DebugInfoSenderMixin {
	...
	public static void sendGameEventListener(World world, GameEventListener eventListener) {
		...
		quilt$send(
			VanillaDebugFeatures.GAME_EVENT_LISTENERS,
			new GameEventListenersDebugPayload(eventListener.getPositionSource(), eventListener.getRange()), (ServerWorld) world
		);
	}
	...
	* **/
	...
	public static void sendGoalSelector(World world, MobEntity mob, GoalSelector goalSelector) {
		...
		goalSelector.getGoals().forEach(goal -> {
			goals.add(new net.minecraft.network.packet.s2c.payload.GoalSelectorDebugPayload.Goal(
				goal.getPriority(),
				goal.isRunning(),
				goal.toString()
			));
		...
		quilt$send(
			VanillaDebugFeatures.GOAL_SELECTOR,
			new GoalSelectorDebugPayload(
				mob.getId(),
				mob.getBlockPos(),
				goals
			),
	...
	public static void sendRaids(ServerWorld server, Collection<Raid> raids) {
		...
		raids.forEach(raid -> {
			centers.add(raid.getCenter());
		});

		quilt$send(
			VanillaDebugFeatures.RAID,
